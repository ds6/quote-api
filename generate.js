...
module.exports = async parm => {
	...
	for (const key in parm.messages) {
		...
		if (message) {
			const canvasQuote = await quoteGenerate.generate(backgroundColorOne, backgroundColorTwo, message, parm.width, parm.height, parseFloat(parm.scale), parm.emojiBrand);
	...
	if (type === 'quote') {
		...
		const gradient = canvasPicCtx.createRadialGradient(canvasPic.width / 2, canvasPic.height / 2, 0, canvasPic.width / 2, canvasPic.height / 2, canvasPic.width / 2);
