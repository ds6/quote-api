...
class QuoteGenerate {
	...
	async downloadMediaImage(media, mediaSize, type = 'id', crop = true) {
		let mediaUrl;
		if (type === 'id') mediaUrl = await this.telegram.getFileLink(media).catch();
		else mediaUrl = media;
	...
	async drawMultilineText(text, entities, fontSize, fontColor, textX, textY, maxWidth, maxHeight, emojiBrand = 'apple') {
		...
		const getCustomEmojiStickers = await this.telegram
			.callApi('getCustomEmojiStickers', {
		...
		for (let index = 0; index < styledWords.length; index++) {
			...
			if (styledWord.emoji) {
				if (styledWord.customEmojiId && customEmojiStickers[styledWord.customEmojiId]) {
					...
					if (!emojiImage) {
						emojiImage = await loadImage(Buffer.from(fallbackEmojiImageJson[styledWord.emoji.code], 'base64')).catch(() => {});
					}
				}
			}
	...
	/**
	 * Draws a WhatsApp-style speech bubble.
	 *
	 * The tail is part of the same path as the rounded rectangle so there
	 * are no seams. The bubble itself starts at x = tailWidth, allowing the
	 * tail to extend into the avatar area when the result is composited.
	 */
	drawChatBubble(colorOne, colorTwo, w, h, r, tailWidth = 18, tailHeight = 10, scale = 1) {
		const canvas = createCanvas(w + tailWidth, h + tailHeight);
		...
		const gradient = ctx.createLinearGradient(x, y, x + w, h);
		gradient.addColorStop(0, colorOne);
		gradient.addColorStop(1, colorTwo);
		...
		// Bottom edge and curved speech tail.
		ctx.lineTo(x + 34 * scale, y + h);
		ctx.bezierCurveTo(x + 25 * scale, y + h, x + 15 * scale, y + h + tailHeight, x - tailWidth + 2 * scale, y + h + tailHeight);
		ctx.bezierCurveTo(x - 9 * scale, y + h + 4 * scale, x - 3 * scale, y + h - 1 * scale, x, y + h - 12 * scale);
	...
	async drawQuote(scale = 1, backgroundColorOne, backgroundColorTwo, avatar, replyName, replyNameColor, replyText, name, text, media, mediaType, maxMediaSize) {
		...
		// Leave enough room for the speech tail below the bubble.
		const bubbleExtraHeight = 10 * scale;
		const canvas = createCanvas(width, height + bubbleExtraHeight);
		const canvasCtx = canvas.getContext('2d');
		...
		// Larger radius gives the much rounder look from the reference.
		const rectRoundRadius = 28 * scale;
		const bubbleTailWidth = 22 * scale;
		const bubbleTailHeight = 10 * scale;
		...
		if (mediaType !== 'sticker' || name || replyName) {
			rect = this.drawChatBubble(backgroundColorOne, backgroundColorTwo, rectWidth, rectHeight, rectRoundRadius, bubbleTailWidth, bubbleTailHeight, scale);
		}

		// Draw the bubble first. The avatar sits over its lower-left corner,
		// matching the WhatsApp-like overlap in the reference image.
		if (rect) canvasCtx.drawImage(rect, rectPosX - bubbleTailWidth, rectPosY);

		const avatarPosY = Math.max(0, rectPosY + rectHeight - avatarSize);
	...
	async generate(backgroundColorOne, backgroundColorTwo, message, width = 512, height = 512, scale = 2, emojiBrand = 'apple') {
		...
		let textCanvas;
		if (message.text) {
			textCanvas = await this.drawMultilineText(message.text, message.entities, fontSize, textColor, 0, fontSize, width, height - fontSize, emojiBrand);
		}
		...
		if (message.replyMessage && message.replyMessage.name && message.replyMessage.text) {
			...
			if (message.replyMessage.name) {
				replyName = await this.drawMultilineText(message.replyMessage.name, 'bold', replyNameFontSize, replyNameColor, 0, replyNameFontSize, width * 0.9, replyNameFontSize, emojiBrand);
			}
			...
			replyText = await this.drawMultilineText(message.replyMessage.text, message.replyMessage.entities, replyTextFontSize, textColor, 0, replyTextFontSize, width * 0.9, replyTextFontSize, emojiBrand);
		}
		...
		const quote = this.drawQuote(scale, backgroundColorOne, backgroundColorTwo, avatarCanvas, replyName, replyNameColor, replyText, nameCanvas, textCanvas, mediaCanvas, mediaType, maxMediaSize);
